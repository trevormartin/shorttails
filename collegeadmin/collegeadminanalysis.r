...
ggplot(data=adminstudgrwclean) + geom_point(aes(x=netadminfc,y=studfc),alpha=.8,size=5) + theme_bw(base_size=25) + scale_x_continuous(breaks=seq(0,45,5),limits=c(0,45)) + scale_y_continuous(breaks=seq(0,45,5),limits=c(0,45)) + theme(axis.line = element_line(colour = "black"),panel.grid.minor = element_blank(),panel.border = element_blank()) + geom_abline(intercept=0,slope=1,col="red") + geom_point(data=adminstudmean,aes(x=adminmean,y=studmean),col="red",size=6,pch=15)
dev.off()
